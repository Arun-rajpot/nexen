...
def get_full_md5_hash(url: str) -> str:
    return hashlib.md5(url.encode("utf-8")).hexdigest()

def request_captcha_link(link,captcha):
    # timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = f"D:\\DistictCaptcha\\resolved_captchaset\\{captcha}.png"
    ...
    # captcha_text = predict_captcha(response.content,)
    # print(captcha_text)
    if response.status_code == 200:
        with open(save_path, "wb") as f:
            f.write(response.content)
        print("Captcha saved successfully:", save_path)
    else:
        print("Failed to fetch captcha:", response.status_code)
...
class DcourtScraper:
    ...
    def process_data_payload(self, data_soup):
        try:
            ...
            print(captcha_src)
            captcha_Naame =  get_full_md5_hash(captcha_src)
            request_captcha_link(captcha_src,captcha_Naame)
            print("===="*30)
            # time.sleep(5)
    ...
    def scrape(self):
        try:
            ...
            district_url = f"https://ecourts.gov.in/ecourts2.0//?p=dist_court/{self.state}&app_token="
            ...
            for district in district_list:
                ...
                for est_code_option in est_code_options:
                    self.process_data_payload(data_soup)
